...
def auto_grade_pending_bets(sheet, odds_key):
    """Fetches completed WNBA scores and updates PENDING rows safely in batch."""
    try:
...
def load_memory():
    if os.path.exists("wnba_bot_memory.json"):
        try:
            with open("wnba_bot_memory.json", "r") as f:
                return json.load(f)
        except Exception:
            pass
...
def main():
    ...
    print("Successfully published adaptive WNBA picks!")
