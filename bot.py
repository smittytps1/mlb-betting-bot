...
def ensure_headers(sheet):
  ...
  try:
    ...
    if len(existing_rows) == 0 or (
        len(existing_rows) > 0 and existing_rows[0][0] != "Date"
    ):
...
def ensure_evolution_sheet(spreadsheet):
  ...
  try:
    try:
      ...
      evo_sheet = spreadsheet.add_worksheet(
          title="Evolution & Learnings", rows=100, cols=10
      )
    ...
    headers = [
        ...
        "Win Rate (%)",
        "Net Profit ($)",
        "Reasoning Factor Weights",
        "Active Strategy Adjustment",
        "Validation & Re-Synthesis Notes",
    ]
...
def update_evolution_log(
    spreadsheet, sport_label, memory, validations_summary, current_time_str
):
  ...
  try:
    ...
    weights_str = " | ".join(
        [f"{k}: {v.get('weight', 1.0)}x" for k, v in factors.items()]
    )
    ...
    evo_sheet.append_row([
        ...
        (
            validations_summary
            if validations_summary
            else "No re-synthesis needed."
        ),
    ])
...
# ==============================================================================
# 2. ACCURATE AUTO-GRADING (MONEYLINE, SPREAD, & TOTALS)
# ==============================================================================
def auto_grade_pending_bets(sheet, odds_key):
  ...
  try:
    ...
    for row_idx, r in enumerate(rows[1:], start=2):
      if (
          len(r) > status_idx
          and str(r[status_idx]).strip().upper() == "PENDING"
      ):
    ...
    if resp.status_code != 200:
      print(f"Could not fetch score data. Code: {resp.status_code}")
      return
    ...
    for row_idx, r in pending_rows:
      ...
      try:
        units = (
            float(r[units_idx])
            if len(r) > units_idx and r[units_idx]
            else 1.0
        )
      except (ValueError, TypeError):
        units = 1.0
      ...
      try:
        clean_time = (
            pulled_time_raw.replace(" EDT", "").replace(" EST", "").strip()
        )
        pulled_dt = datetime.strptime(
            clean_time, "%Y-%m-%d %H:%M:%S"
      ...
      for match in scores_data:
        ...
        if home_team in game_title or away_team in game_title:
          ...
          home_score = next(
              (int(s["score"]) for s in scores if s["name"] == home_team), 0
          )
          away_score = next(
              (int(s["score"]) for s in scores if s["name"] == away_team), 0
          )
          ...
          if (
              "total" in bet_type
              or "over" in pick_str.lower()
              or "under" in pick_str.lower()
          ):
            ...
            if num_match:
              ...
              if total_score == total_line:
                status = "PUSH"
                profit = 0.0
              elif (is_over and total_score > total_line) or (
                  not is_over and total_score < total_line
              ):
                status = "WIN"
              else:
                status = "LOSS"
          ...
          # Moneylines
            ...
            is_win = (
                pick_str.lower() in winner.lower()
                or winner.lower() in pick_str.lower()
            )
            status = "WIN" if is_win else "LOSS"

          if status == "WIN":
            profit = (
                (100 / abs(odds)) * 100 * units
                if odds < 0
                else (odds / 100) * 100 * units
            )
          elif status == "LOSS":
            profit = -100.0 * units
          elif status == "PUSH":
          ...
          print(
              f"Graded Row {row_idx}: {game_title} [{pick_str}] ->"
              f" {status} (${round(profit, 2)})"
          )
          updates.append({
...
def update_scoreboard(spreadsheet):
  try:
    try:
      ...
      sb_sheet = spreadsheet.add_worksheet(
          title="Scoreboard", rows=20, cols=10
      )
    ...
    scoreboard_data = [
        ...
        [
            "MLB Bot",
            '=COUNTIF(MLB!K:K, "WIN")',
            '=COUNTIF(MLB!K:K, "LOSS")',
            '=COUNTIF(MLB!K:K, "PENDING")',
            "=IFERROR(B2/(B2+C2), 0)",
            "=SUM(MLB!L:L)",
        ],
        [
            "WNBA Bot",
            '=COUNTIF(WNBA!K:K, "WIN")',
            '=COUNTIF(WNBA!K:K, "LOSS")',
            '=COUNTIF(WNBA!K:K, "PENDING")',
            "=IFERROR(B3/(B3+C3), 0)",
            "=SUM(WNBA!L:L)",
        ],
        [
    ...
    sb_sheet.update(
        range_name="A1:F4",
        values=scoreboard_data,
        value_input_option="USER_ENTERED",
    )
    sb_sheet.format("A1:F1", {"textFormat": {"bold": True}})
    sb_sheet.format(
        "E2:E4", {"numberFormat": {"type": "PERCENT", "pattern": "0.0%"}}
    )
    sb_sheet.format(
        "F2:F4", {"numberFormat": {"type": "CURRENCY", "pattern": "$#,##0.00"}}
    )
    print("Scoreboard tab updated with live formulas!")
  except Exception as e:
    print(f"Notice while updating Scoreboard: {e}")


# ==============================================================================
# 4. RECURSIVE MEMORY & FACTOR WEIGHTING ENGINE
# ==============================================================================
def load_memory():
  ...
  default_memory = {
      ...
      "learnings_and_adjustments": (
          "Maintain balanced multi-factor evaluation."
      ),
      "reasoning_factor_weights": {
          "starting_pitcher_metrics": {
              "wins": 0,
              "losses": 0,
              "weight": 1.0,
              "instruction": (
                  "Evaluate xFIP, SIERA, CSW%, K-BB%, and handedness splits."
              ),
          },
          "pitch_mix_arsenal_matchup": {
              "wins": 0,
              "losses": 0,
              "weight": 1.0,
              "instruction": (
                  "Evaluate pitcher pitch mix vs lineup pitch tracking run"
                  " values (RV/100)."
              ),
          },
          "bullpen_usage_and_leverage": {
              "wins": 0,
              "losses": 0,
              "weight": 1.0,
              "instruction": (
                  "Evaluate 3-day rolling pitch counts, high-leverage tier FIP,"
                  " and middle relief."
              ),
          },
          "statcast_offensive_splits": {
              "wins": 0,
              "losses": 0,
              "weight": 1.0,
              "instruction": (
                  "Evaluate wRC+/wOBA vs pitcher hand, Barrel%, Hard-Hit%,"
                  " and chase/walk rates."
              ),
          },
          "ballpark_and_weather_physics": {
              "wins": 0,
              "losses": 0,
              "weight": 1.0,
              "instruction": (
                  "Evaluate park factors, wind vectors, air density, temp, and"
                  " roof status."
              ),
          },
          "defense_umpire_and_rest": {
              "wins": 0,
              "losses": 0,
              "weight": 1.0,
              "instruction": (
                  "Evaluate DRS/OAA, catcher framing, umpire zone size, and"
                  " travel/rest spots."
              ),
          },
      },
...
def calculate_factor_weight(wins, losses):
  ...
  win_rate = wins / total
  if win_rate >= 0.65:
    return min(1.5, round(1.0 + (win_rate - 0.5) * 1.0, 2)), (
        f"High win rate ({round(win_rate*100, 1)}%). Prioritize this factor"
        " when establishing model edge."
    )
  elif win_rate <= 0.40:
    return max(0.3, round(1.0 - (0.5 - win_rate) * 1.2, 2)), (
        f"Cold streak ({round(win_rate*100, 1)}%). De-emphasize as a primary"
        " driver; use only as secondary support."
    )
  else:
    return (
        1.0,
        f"Neutral performance ({round(win_rate*100, 1)}%). Standard weighting.",
    )


def update_memory_from_sheet(sheet, memory):
  """Scans graded bet reasoning to dynamically calibrate factor weights based on success/failure."""
  try:
    rows = sheet.get_all_values()
    if len(rows) <= 1:
      return memory
    ...
    wins = sum(
        1
        for r in rows[1:]
        if len(r) > status_idx and str(r[status_idx]).strip().upper() == "WIN"
    )
    losses = sum(
        1
        for r in rows[1:]
        if len(r) > status_idx and str(r[status_idx]).strip().upper() == "LOSS"
    )
    total = wins + losses
    ...
    if total > 0:
      ...
      net_pl = sum(
          float(r[pl_idx] or 0.0)
          for r in rows[1:]
          if len(r) > pl_idx and r[pl_idx]
      )
      ...
      if win_rate < 50.0:
        memory["learnings_and_adjustments"] = (
            f"Win rate is {win_rate}% (<50%). Raising EV threshold to +10%"
            " and de-emphasizing low-weight factor drivers."
        )
      else:
        memory["learnings_and_adjustments"] = (
            f"Win rate is {win_rate}% (profitable). Maintain current"
            " quantitative multi-factor selection criteria."
        )
...
# ==============================================================================
# 6. QUANTITATIVE SYNTHESIS & PICK GENERATION (GEMINI ENGINE)
# ==============================================================================
def generate_picks_and_validations(odds_data, memory, open_picks):
  print(
      "Sending odds data, multi-factor weights, open picks, and memory to"
      " Gemini..."
  )
  ...
  prompt = f"""
    ...
    1. STARTING PITCHING:
       - Rely on predictive metrics (xFIP, SIERA, CSW%, K-BB%) over surface ERA.
       - Assess handedness splits (wOBA, K%, Hard-Hit% allowed vs LHB/RHB).
       - Evaluate pitch arsenal (RV/100) vs opposing lineup tracking tendencies.
       - Check velocity deviations (>0.8 mph drop) and 3-start workloads.
    2. BULLPEN DEPTH & USAGE:
       - 3-day rolling pitch counts; flag fatigued high-leverage arms.
       - Compare leverage-tier FIP/WHIP against middle-relief vulnerabilities.
    3. OFFENSIVE PROFILE & LINEUP SPLITS:
       - Split-adjusted wRC+ and wOBA vs opposing pitcher handedness.
       - Statcast contact quality: Barrel%, Hard-Hit% (95+ mph), xwOBA.
       - Plate discipline: Walk rates (BB%) and chase rates.
    4. BALLPARK & ENVIRONMENTAL PHYSICS:
       - Venue-specific park factors for runs/HRs.
       - Wind vector (speed/direction relative to home-to-center field).
       - Temperature, relative humidity, and air density effects on ball carry.
       - Roof status (open vs closed).
    5. DEFENSE, UMPIRE & SITUATIONAL SPOTS:
       - DRS and OAA (middle infield, CF, catcher framing).
       - Home plate umpire zone tendencies (high-K vs high-BB).
       - Travel, time zones, rest disparity, and series context.
    ...
    - If you find compelling evidence opposing an active open pick, combine the arguments for BOTH sides (applying factor weights) to establish one true stance.
  ...
  for attempt in range(max_retries):
    try:
      ...
      clean_json = (
          json_match.group(0)
          if json_match
          else text.replace("```json", "").replace("```", "").strip()
      )
      ...
      if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
        ...
        print(
            f"Rate limit (429) hit. Retrying in {wait_time}s (Attempt"
...
def is_duplicate_pick(raw_rows, pick_date, game, bet_type, pick, odds_val):
  ...
  for r in raw_rows[1:]:
    ...
    if (
        r_date == pick_date
        and r_teams == norm_teams
        and r_bet_type == norm_bet_type
        and r_pick == norm_pick
        and r_odds == norm_odds
    ):
      return True
...
def main():
  ...
  today_date_str = datetime.now(ZoneInfo("America/New_York")).strftime(
      "%Y-%m-%d"
  )
  current_time_str = datetime.now(ZoneInfo("America/New_York")).strftime(
      "%Y-%m-%d %H:%M:%S EDT"
  )

  print(
      f"Memory Loaded | Total Bets: {updated_memory['total_bets']} | Win Rate:"
      f" {updated_memory['win_rate']}"
  )
  ...
  for p in new_picks:
    ...
    if is_duplicate_pick(raw_rows, pick_date, game, bet_type, pick, odds_val):
      print(
          f"Skipping duplicate prediction: {game} | {pick} @"
          f" {int(round(odds_val))}"
      )
      skipped_count += 1
      continue
  ...
  val_summary_str = (
      " ; ".join(val_notes)
      if val_notes
      else f"Generated {appended_count} new pick(s)."
  )
  update_evolution_log(
      spreadsheet, "MLB", updated_memory, val_summary_str, current_time_str
  )

  print(
      f"MLB Execution Complete: {appended_count} new/updated pick(s) added,"
